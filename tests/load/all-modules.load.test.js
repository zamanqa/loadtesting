/**
 * Load test — All modules combined (44 GET endpoints)
 *
 * Purpose : Measure baseline API performance under normal operating load.
 *           Every GET endpoint across all 10 modules fires in sequence per VU
 *           iteration. Thresholds are set from measured p95 values at 60 VUs
 *           plus 20% headroom — search endpoints are slowest under concurrency.
 *
 * VU profile (3-stage ramp — default 5 VUs, edit stages to scale up):
 *    0 → 5 VUs  (30s)  — ramp up
 *       5 VUs   (30s)  — hold steady
 *       0 VUs   (30s)  — ramp down
 *   Total: ~1.5 min
 *
 * To scale up: increase target VUs and stage durations in options.scenarios.load.stages.
 * Thresholds are calibrated for 60 VUs; the default 5 VU run is for quick verification.
 *
 * Modules covered:
 *   Orders              (6 endpoints)
...
const ENDPOINTS = [
  ...
  // Draft Orders — all under 1300ms at 60 VUs
...
export default function (data) {
  ...
  const params = makeParams(token, 'load', k6.REQUEST_TIMEOUT);
...
export function teardown(data) {
  console.log('[load] All-modules load test complete — 44 endpoints measured.');
}
...
export function handleSummary(data) {
  return {
    'tests/load/reports/all-modules-load-report.html': buildHtmlReport(data, REPORT_CONFIG),
